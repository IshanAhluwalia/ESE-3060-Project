...
class Conv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 padding='same', bias=False, stride=1):
        # Handle padding manually when stride > 1
...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        super().__init__()
        # Learnable downsampling — improves gradient flow + runtime
        ...
        # Faster + often higher CIFAR accuracy
        self.activ = nn.SiLU()

    def forward(self, x):
        x = self.conv1(x)        # ↓ feature map here
        x = self.norm1(x)
        x = self.activ(x)
