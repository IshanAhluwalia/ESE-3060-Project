...
class ConvGroup(nn.Module):
    def __init__(self, channels_in, channels_out, batchnorm_momentum):
        super().__init__()
        # Downsample using stride-2 conv instead of MaxPool
        self.conv1 = Conv(channels_in, channels_out, kernel_size=3, stride=2, padding=1)
        self.pool = nn.Identity()  # no-op, keeps the interface the same
        self.norm1 = BatchNorm(channels_out, batchnorm_momentum)
        self.conv2 = Conv(channels_out, channels_out)
        self.norm2 = BatchNorm(channels_out, batchnorm_momentum)
        self.activ = nn.GELU()     # keep GELU for this ablation
